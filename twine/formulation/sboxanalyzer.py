...
class SboxAnalyzer(SBox):
    ...
    def _compute_data_for_difflin_analysis(self):
        ...
        self._dlct = [[0 for y in range(2**output_size)] for x in range(2**input_size)]
            ...
            for mask in range(2**output_size):
                for data in range(2**input_size):
                    if self.inner_product(mask, self[data]) == self.inner_product(mask, self[data ^ diff]):
                        self._dlct[diff][mask] += 1
        self._dlct = [[self._dlct[x][y] - 2**(input_size - 1) for y in range(2**output_size)] for x in range(2**input_size)]
...
if __name__ == "__main__":
    ...
    from sage.crypto.sboxes import SBox
    from sage.crypto.sboxes import Midori_Sb0 as sb
    ...
    # mpt = sa.compute_monomial_prediction_table()
